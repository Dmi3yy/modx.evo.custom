				...
				init: function() {
					...
					for(i = 0; i < els.length; i++) {
						els[i].onmouseover = function(e) {
							...
							for(ii = 0; ii < els.length; ii++) {
								if(els[ii].classList.contains('show'))
									els[ii].classList.remove('show')
							}
							if(this.parentNode.classList.contains('dropdown-menu') && this.firstChild.tagName === 'A') {
								var a = this.firstChild;
								var href = a.href.split('?')[1];
								if(parseInt(modx.main.getQueryVariable('a', '&' + href)) === 76) {
									// e.preventDefault();
									// e.stopPropagation();
									var sub = this.parentNode.parentNode.children['parent_' + this.id];
									if(sub) {
										...
										modx.post('/' + modx.MGR_DIR + '/media/style/' + modx.config.theme + '/ajax.php', href + '&parent=' + el.id, function(r) {
											if(r) {
												...
												ul.innerHTML = r;
												ul.style.left = el.parentNode.offsetWidth + 'px';
												ul.id = 'parent_' + el.id;
