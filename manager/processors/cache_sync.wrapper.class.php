...
abstract class documentCacheBase implements Iterator, Countable {
	...
	protected function getRecord($key){
		...
		if(!$offset = $this->findOffset((int)$key)) return false;
		$tmpcache = substr($this->cache, $offset , 3);
		$len = $this->bin2dec($tmpcache,3);
		unset($tmpcache);
		return substr($this->cache, $offset + 3, $len);}
	...
	protected function findOffset($key){$left = 1; $right = $this->records; while ($left <= $right){ $middle = (int)(($left+$right)/2); 
	$tmpcache = substr($this->cache,$middle*8 - 4,4);
	$ckey = (int)$this->bin2dec($tmpcache,4);
	unset($tmpcache);
	if(($left == $right) && ($ckey != $key)) return false;
	else if($ckey == $key){
		$tmpcache = substr($this->cache, $middle*8,4);
		return $this->bin2dec($tmpcache,4) + $this->dataStart;
	}
	else if($key < $ckey) $right = $middle - 1; else if($key > $ckey) $left = $middle + 1;} return false;}
}
...
class AliasListingClass extends documentCacheBase implements ArrayAccess {
	...
	$this->cache = $cache;
	$tmpcache = substr($cache,0,4); $this->records = $this->bin2dec($tmpcache,4); unset($tmpcache);
	$this->dataStart = $this->records * 8 + 4;$this->cacheLen = strlen($cache);}
...
class DocumentListingClass implements ArrayAccess, Iterator, Countable {
	...
	public function __construct($cache){
		$this->cache = $cache;
		$tmpcache = substr($cache,0,4); $this->records = $this->bin2dec($tmpcache,4); unset($tmpcache);
		$this->dataStart = $this->records * 4 + 4;
		$this->cacheLen = strlen($cache);
	}
    public function rewind() {$this->pointer = $this->dataStart; $this->curRecord = 0;}
	...
	protected function bin2dec(&$str, $len){$shift = 0;$result = 0;for($i=0; $i<$len; ++$i){$result |= (@ord($str[$i])<<$shift);$shift += 8;}return $result;}
	protected function getRecord($key){
		if(!$key) return false; if(!$offset = $this->findOffset($key)) return false;
		$tmpcache = substr($this->cache, $offset , 2);
		$len = $this->bin2dec($tmpcache,2);
		unset($tmpcache);
		$tmpcache = substr($this->cache, $offset + $len + 2, 4);
		return (int)$this->bin2dec($tmpcache, 4);}
	...
	protected function findOffset($key){
		$key = (string)$key; $left = 1; $right = $this->records; 
		while ($left <= $right){$middle = (int)(($left+$right)/2);
		$tmpcache = substr($this->cache,$middle * 4 ,4);
		$idxoffs = $this->bin2dec($tmpcache,4);
		unset($tmpcache);
		$tmpcache = substr($this->cache, $this->dataStart + $idxoffs, 2);
		$len = $this->bin2dec($tmpcache,2); 
		unset($tmpcache);
		$ckey = substr($this->cache, $this->dataStart + $idxoffs + 2, $len); if(($left == $right) && ((string)$ckey != (string)$key)) return false; else if((string)$ckey == (string)$key) return $idxoffs + $this->dataStart; else if((string)$key < (string)$ckey) $right = $middle - 1; else if((string)$key > (string)$ckey) $left = $middle + 1;} return false;}
